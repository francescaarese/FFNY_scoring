...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 14 12:02:55 2025
...
weights = {
    ...
    'Company Growth Score': st.sidebar.slider("Company Growth Score Weight", 0.0, 1.0, 0.15, 0.01),
...
def recent_financing(company, reference_date_str):
    ...
    if pd.notna(last_financing_date) and last_financing_date > reference_date - timedelta(days=730):
        ...
        if pd.notna(last_financing_size) and last_financing_size >= 200:
            large_financing = 5
...
def check_hq_location(company):
    ...
    if region in ['south america','central america']:
        return 10
    if region in ['canada', 'north america']:
        return 5
    else:
        return 0
